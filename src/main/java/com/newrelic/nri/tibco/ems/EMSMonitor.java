...
//	private static final Logger logger = LoggerFactory.getLogger(EMSMonitor.class);
	...
	private TibjmsAdmin connectWithSSL(EMSServer emsServer) {
		TibjmsAdmin tibcoInst = null;
		String user = emsServer.getUsername();
		String host = emsServer.getEMSURL();
		String password = emsServer.getPassword();
		Map<String, Object> sslParams = new HashMap<String, Object>();

		String sslIdentityFile = emsServer.getSslIdentityFile();
		String sslIdentityPassword = emsServer.getSslIdentityPassword();
		String sslTrustedCerts = emsServer.getSslTrustedCerts();
		...
		String sslCiphers = emsServer.getSslCiphers();
		Boolean sslDebug = emsServer.getSslDebug();
		Boolean sslVerifyHost = emsServer.getSslVerifyHost();
		Boolean sslVerifyHostName = emsServer.getSslVerifyHostName();
		String sslVendor = emsServer.getSslVendor();
		...
		try {
			tibcoInst = new TibjmsAdmin(host, user, password, sslParams);
		} catch (TibjmsAdminException e) {
			Utils.reportError("Failed to connect", e);
		}
	...
	public void populateMetrics(JSONMetricReporter metricReporter) throws Exception {
		TibjmsAdmin tibjmsServer = null;
		if (emsServer.isUseSSL()) {
			tibjmsServer = connectWithSSL(emsServer);
		} else {

			tibjmsServer = connect(emsServer);
		}
						...
						if (details != null && details.length > 0) {
							for (DetailedDestStat detail : details) {
								...
								if (inboundStat != null) {
									...
									metricList
											.add(new RateMetric("Inbound-Message Rate", inboundStat.getMessageRate()));
									addGaugeMetric(metricList, "Inbound-Total Bytes", inboundStat.getTotalBytes());
									addGaugeMetric(metricList, "Inbound-Total Messages",
											inboundStat.getTotalMessages());
								}
								...
								if (outboundStat != null) {
									...
									metricList.add(
											new RateMetric("Outbound-Message Rate", outboundStat.getMessageRate()));
									addGaugeMetric(metricList, "Outbound-Total Bytes", outboundStat.getTotalBytes());
									addGaugeMetric(metricList, "Outbound-Total Messages",
											outboundStat.getTotalMessages());
								}
								metricList.add(new AttributeMetric("EMS Server", emsServerName));
		...
		try {
			...
			if (queueList != null)
				for (QueueInfo q : queueList) {
					...
					if (q.isTemporary() && !emsServer.getFlagIncludeDynamicQueues())
						skip = true;
					...
					if (qName.toUpperCase().startsWith(TMPSTART))
						qName = TEMP;
					if (qName.toUpperCase().startsWith(SYSSTART))
						qName = SYS;
					...
					if (!skip) {
						...
						if (q.getInboundStatistics() != null) {
							...
							metricList.add(
									new RateMetric("Inbound-Message Rate", q.getInboundStatistics().getMessageRate()));
							metricList.add(
									new RateMetric("Inbound-Total Bytes", q.getInboundStatistics().getTotalBytes()));
							metricList.add(new RateMetric("Inbound-Total Messages",
									q.getInboundStatistics().getTotalMessages()));
						}

						if (q.getOutboundStatistics() != null) {
							metricList
									.add(new RateMetric("Outbound-Byte Rate", q.getOutboundStatistics().getByteRate()));
							metricList.add(new RateMetric("Outbound-Message Rate",
									q.getOutboundStatistics().getMessageRate()));
							addGaugeMetric(metricList, "Outbound-Total Bytes",
									q.getOutboundStatistics().getTotalBytes());
							addGaugeMetric(metricList, "Outbound-Total Messages",
									q.getOutboundStatistics().getTotalMessages());
						}
...
//			int totalDurableSubscriptions = 0;
			...
			if (topicList != null) {
				for (TopicInfo t : topicList) {
					...
					if (t.isTemporary() && !emsServer.getFlagIncludeDynamicTopics())
						skip = true;
					...
					if (tName.toUpperCase().startsWith(TMPSTART))
						tName = TEMP;
					if (tName.toUpperCase().startsWith(SYSSTART))
						tName = SYS;
...
//						totalSubscriptionCount += subscriptionCount;
						if (t.getInboundStatistics() != null) {
							...
							metricList.add(
									new RateMetric("Inbound-Message Rate", t.getInboundStatistics().getMessageRate()));
							addGaugeMetric(metricList, "Inbound-Total Bytes", t.getInboundStatistics().getTotalBytes());
							addGaugeMetric(metricList, "Inbound-Total Messages",
									t.getInboundStatistics().getTotalMessages());
						}

						if (t.getOutboundStatistics() != null) {
							metricList
									.add(new RateMetric("Outbound-Byte Rate", t.getOutboundStatistics().getByteRate()));
							metricList.add(new RateMetric("Outbound-Message Rate",
									t.getOutboundStatistics().getMessageRate()));
							addGaugeMetric(metricList, "Outbound-Total Bytes",
									t.getOutboundStatistics().getTotalBytes());
							addGaugeMetric(metricList, "Outbound-Total Messages",
									t.getOutboundStatistics().getTotalMessages());
						}
						metricList.add(new AttributeMetric("EMS Server", emsServerName));
			...
			if (routeList != null)
				for (RouteInfo t : routeList) {
					...
					if (t.getInboundStatistics() != null) {
						metricList.add(new RateMetric("Inbound-Byte Rate", t.getInboundStatistics().getByteRate()));
						metricList
								.add(new RateMetric("Inbound-Message Rate", t.getInboundStatistics().getMessageRate()));
